...
function tryStatSync(path4) {
  try {
    ...
    return new import_node_fs.Stats();
  }
}
function fileExists(url) {
  const stats = (0, import_node_fs.statSync)(url, { throwIfNoEntry: false });
...
function packageImportsResolve(name, base, conditions) {
  ...
  if (packageConfig.exists) {
    ...
    if (imports2) {
      if (own.call(imports2, name) && !name.includes("*")) {
        const resolveResult = resolvePackageTarget(
          packageJsonUrl,
          imports2[name],
          "",
...
function _resolve(id, options = {}) {
  ...
  if (isAbsolute(id) && (0, import_node_fs.existsSync)(id)) {
    const realPath2 = (0, import_node_fs.realpathSync)(fileURLToPath(id));
  ...
  const realPath = (0, import_node_fs.realpathSync)(fileURLToPath(resolved));
...
function lincy(options = {}, ...userConfigs) {
  const {
    isInEditor = !!((import_node_process4.default.env.VSCODE_PID || import_node_process4.default.env.JETBRAINS_IDE) && !import_node_process4.default.env.CI),
